...
public class Validator {
	public static boolean checkPhoneNumber(String phoneNumber) throws PhoneException {
		Pattern pattern = Pattern.compile(Constants.VALID_PHONE_REGEX);
		Matcher matcher = pattern.matcher(phoneNumber);
		if (matcher.matches()) {
			return true;
		} else {
			throw new PhoneException("Sai dinh dang !");
	...
	public static boolean checkBirthday(String date) throws BirthDayException {
		...
		try {
			...
			if (date1.getYear() < 1900 || date1.getYear() > LocalDateTime.now().getYear()
				...
				throw new BirthDayException("Sai dinh dang");
			}
		} catch (DateTimeParseException e) {
			throw new BirthDayException("Sai dinh dang dd/MM/yyyy");
		}
